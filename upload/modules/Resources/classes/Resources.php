...
class Resources {
    ...
    public function canDownloadResourceFromCategory($group_ids, $cat_id){
        ...
        return $this->_db->query('SELECT `view`, `download` FROM nl2_resources_categories_permissions WHERE category_id = ? AND `download` = 1 AND `post` = 1 AND group_id IN (' . $group_ids . ')', array($cat_id))->count() ? true : false;
    }
